...
Built with **Python, FastAPI, Playwright, and Claude** (via the Anthropic API, with an interchangeable AWS Bedrock planner), using typed Pydantic models throughout for the artifact and result contracts.

## What is implemented

- Goal-driven LLM observe -> decide -> act discovery loop using Claude (via the Anthropic API or, interchangeably, AWS Bedrock).
...
In terminal 2, with your LLM credentials configured (see Section 1 above):
...
**Evidence included:** this repository already contains a genuine LLM-driven discovery run against the local live UI — `evidence/discovery_live.jsonl` (step-by-step model decisions) and `evidence/discovered_capability.json` (the resulting artifact). Re-running the command above with your own API key will reproduce a fresh run; it is not required to trust the included evidence.
...
**Note:** the commands above replay the static example artifact. The checked-in `evidence/replay_success.jsonl` and `evidence/replay_not_found.jsonl` were generated by replaying the same two inputs against `evidence/discovered_capability.json` — the artifact produced by the actual discovery run in Section 3 — which is the stronger, non-static evidence.
...
```text
app/agent/          Anthropic/Bedrock planners and discovery loop
...
The end-to-end thread has been run for real: a live discovery run against the local UI (`evidence/discovery_live.jsonl`, `evidence/discovered_capability.json`), followed by two deterministic replays of that same artifact — one success, one `MEMBER_NOT_FOUND` business outcome (`evidence/replay_success.jsonl`, `evidence/replay_not_found.jsonl`). `evidence/handoff-request.png` and `evidence/handoff_request.json` are from a genuine escalation triggered during an earlier discovery attempt, kept as evidence the human-in-the-loop path fires for real rather than only existing as a code path.
